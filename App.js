...
const FACTORY_DB = {
  SY: {
    name: "森源",
    shortName: "SY",
    address: "寫字樓--中山市东凤镇东富路尾177号\n裝貨倉庫--中山市东凤镇东富路(在泰峰集团有限公司旁邊)",
    contact: "聯絡人: 劉明, rongxingshoes@163.com 电话：0760--22772481"
  },
  SDL: {
    name: "聖達龍",
    shortName: "SDL",
...
export default function App() {
  const [formData, setFormData] = useState({
    ...
    consignee: 'GLOBE FOOTWEAR CORP.\n190 WESTFIELD AVENUE WEST ROSELLE PARK, NJ 07204',
    notify: 'DSV AIR & SEA, INC.\n100 WALNUT AVENUE, SUITE 405 CLARK, NJ 07066',
    from: 'SHENZHEN, CHINA',
    to: 'NEW YORK, USA',
    orderNo: '1398E, 1513C, 1514C, 1489B',
    marks: 'CS',
    factories: []
  });
      ...
      for (const file of uploadedFiles) {
        try {
          ...
          plSheet.eachRow((row, rowNumber) => {
            ...
            if (rowData[0]) orderSet.add(rowData[0]);
  ...
  const generateExcel = async () => {
    ...
    // 1. 產生 SI Sheet
    // ==========================================
    const siSheet = newWb.addWorksheet('SI', { views:[{ showGridLines: false }] });
    siSheet.columns = Array(9).fill({ width: 16 });

    siSheet.getCell('A1').value = 'SHIPMENT INSTRUCTION';
    siSheet.getCell('A1').font = { size: 14, bold: true, underline: true };
    siSheet.getCell('H1').value = 'PI#';
    siSheet.getCell('I1').value = formData.pi;

    siSheet.getCell('A2').value = `FM: ${formData.fm}`;
    siSheet.getCell('C2').value = `TO: K&A/Sandra`;
    siSheet.getCell('H2').value = 'DATE:';
    siSheet.getCell('I2').value = formData.date;

    siSheet.getCell('A3').value = 'SHIPPER :';
    siSheet.getCell('B3').value = formData.shipper;

    const consLines = formData.consignee.split('\n');
    siSheet.getCell('A4').value = 'CONSIGNEE :';
    siSheet.getCell('B4').value = consLines[0] || '';
    siSheet.getCell('B5').value = consLines.slice(1).join('\n');

    const notifyLines = formData.notify.split('\n');
    siSheet.getCell('A6').value = 'NOTIFY :';
    siSheet.getCell('B6').value = notifyLines[0] || '';
    siSheet.getCell('B7').value = notifyLines.slice(1).join('\n');

    siSheet.getCell('A9').value = `FROM : ${formData.from}`;
    siSheet.getCell('D9').value = `TO : ${formData.to}`;

    siSheet.getCell('A11').value = 'MARKS :';
    siSheet.getCell('A12').value = formData.marks;
    siSheet.getCell('C11').value = `P/I #${formData.pi}`;
    siSheet.getCell('D12').value = `Order No. ${formData.orderNo}`;

    siSheet.getCell('H11').value = 'G.W.(kgs) :';
    siSheet.getCell('I11').value = totals.gw;
    siSheet.getCell('I11').numFmt = '0.00';

    siSheet.getCell('H12').value = 'CBM:';
    siSheet.getCell('I12').value = totals.cbm;
    siSheet.getCell('I12').numFmt = '0.000000';

    siSheet.getCell('H13').value = "Q'NTY(prs) :";
    siSheet.getCell('I13').value = totals.prs;

    siSheet.getCell('H14').value = 'CTNS:';
    siSheet.getCell('I14').value = totals.ctns;

    siSheet.getCell('A15').value = `Cargo Ready Date: ${formData.date}`;
    siSheet.getCell('E15').value = 'Shipping term: FOB SZ';
    siSheet.getCell('B16').value = "1x40'HQ";
    ...
    plSheet.getCell('F4').value = `TO : ${formData.to}`;
  ...
  const isFormValid = formData.pi && formData.date && formData.shipper && uploadedFiles.length > 0;
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-4">
               <div>
                ...
                <input className="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" value={formData.fm} onChange={e => setFormData({...formData, fm: e.target.value})} />
              </div>
               <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Order No. / PO#</label>
                <input className="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" value={formData.orderNo} onChange={e => setFormData({...formData, orderNo: e.target.value})} />
              </div>
            </div>

            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">Shipper</label>
              <input className="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" value={formData.shipper} onChange={e => setFormData({...formData, shipper: e.target.value})} />
            </div>

            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Consignee</label>
                <textarea rows="3" className="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" value={formData.consignee} onChange={e => setFormData({...formData, consignee: e.target.value})} />
              </div>
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-1">Notify</label>
                <textarea rows="3" className="w-full border rounded p-2 focus:ring-2 focus:ring-blue-500 outline-none" value={formData.notify} onChange={e => setFormData({...formData, notify: e.target.value})} />
              </div>
            </div>

            <div className="pt-4 border-t">
              <label className="block text-sm font-semibold text-gray-700 mb-3">客戶資料庫 (帶入報關資訊)</label>
              <div className="grid grid-cols-2 gap-4">
